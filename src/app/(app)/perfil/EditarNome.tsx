...
export function EditarNome({ nomeAtual }: { nomeAtual: string }) {
  const [estado, acao, pendente] = useActionState(atualizarNome, {} as EstadoPerfil);
    ...
    <form action={acao} className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={pendente}
          className="min-h-[44px] rounded-xl bg-[#6C5CE7] px-4 font-medium text-white disabled:opacity-50"
        >
          {pendente ? "..." : "Salvar"}
        </button>
      </div>
      {estado.erro && <p role="alert" className="text-sm text-red-600">{estado.erro}</p>}
      {estado.sucesso && <p className="text-sm text-green-700">Nome atualizado.</p>}
    </form>
  );
}
